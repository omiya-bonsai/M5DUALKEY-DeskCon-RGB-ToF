...
The Matrix does not display absolute distance as discrete steps. With no action it renders a center glow whose brightness, breathing speed, spread, and subtle shimmer follow proximity. Entering the active ToF range starts one armed master-brightness window at 85%, limited to 15 seconds. The newest DualKey, Encoder, or Angle action temporarily returns the master setting to 50% and replaces the ambient frame with a ripple, contraction, expansion, pulse, or directional sweep. Rendering is `millis()`-based and does not block input, HID, or ToF work.
