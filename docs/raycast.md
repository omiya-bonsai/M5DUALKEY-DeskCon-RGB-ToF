...
Raycast handles the keyboard shortcuts emitted by DualKey. The firmware does not directly control ORA4 or Studio Display; it sends keyboard shortcuts, and Raycast maps those shortcuts to commands that run on macOS.
...
| `Ctrl + Option + E` | DualKey both-key custom action |
...
Encoder Volume Up, Volume Down, and Mute bypass Raycast. They are sent directly to macOS as standard USB HID Consumer Control events. Angle scrolling also bypasses Raycast and is sent directly to macOS as USB HID mouse-wheel events. ToF has no Raycast mapping and sends no HID event; its continuous distance only shapes the Matrix ambient light.
